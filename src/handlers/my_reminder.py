...
from src.db.queries.orm import activate_reminder, deactivate_reminder, delete_reminder, select_my_active_daily_reminders, select_my_active_hourly_reminders, select_my_active_one_time_reminders, select_my_active_yearly_reminders, select_my_archive_daily_reminders, select_my_archive_hourly_reminders, select_my_archive_one_time_reminders, select_my_archive_yearly_reminders, select_user, insert_users, update_daily_reminder, update_hourly_reminder, update_onetime_reminder, update_user, update_yearly_reminder
...
async def no_reminders(callback : CallbackQuery, 
                       lang : str,
                       active_archive : str,
                       type : str):
...
def length_of_reminders(len : int) -> int:
    ...
    len_of_reminders = len_of_reminders + 1 if ((len // 5) >= 1 )and ((len % 5) >= 1) else len_of_reminders
...
def texts_of_reminders_with_numbers(lang: str, type: str, reminders, page: int = 0):
    ...
    if type == "onetime":
        for idx, (reminder, local_time) in enumerate(reminders[start:end], start=start + 1):
            text = (
...
async def my_reminders(callback: CallbackQuery):
    ...
    if not reminders:
        await no_reminders(callback=callback,
                     lang=user.language,
    ...
    headline = headline_text(lang=user.language, type="onetime", active_archive="active")
...
async def next_page(callback: CallbackQuery):
    ...
    _, page, type_, active_archive = callback.data.split("_") # nextpage_{page}_{type}_{active_archive}
    page = int(page)


    user = await select_user(callback.from_user.id)
    reminders = await select_my_reminders(tg_id=callback.from_user.id, type = type_, active_archive=active_archive)
    ...
    headline = headline_text(lang=user.language, type=type_, active_archive=active_archive)
...
async def previous_page(callback: CallbackQuery):
    ...
    _, page, type_, active_archive = callback.data.split("_") # next_page_{page}_{type}_{active_archive}
    page = int(page)
...
async def active_reminders(callback : CallbackQuery):
    ...
    _, type_, active_archive = callback.data.split("_") # active_{type}_{active_archive}
    user = await select_user(callback.from_user.id)

    reminders = await select_my_reminders(tg_id=callback.from_user.id, type=type_, active_archive="archive")
...
async def active_reminders(callback : CallbackQuery):
    ...
    reminders = await select_my_reminders(tg_id=callback.from_user.id, type=type_, active_archive="active")
...
async def edit_reminder(callback : CallbackQuery):
    ...
    _, page, type_, active_archive = callback.data.split("_")  # edit_1_{type}_{active_archive}
    page = int(page)

    user = await select_user(callback.from_user.id)
    reminders = await select_my_reminders(tg_id=callback.from_user.id, type=type_, active_archive=active_archive)

    texts = texts_of_reminders_with_numbers(lang=user.language, type=type_, reminders=reminders, page=page-1)
    reminder_text = "\n\n".join(texts[:5])
...
async def next_page(callback: CallbackQuery):
    _, page, type_, active_archive = callback.data.split("_") # editnextpage_{page}_{type}_{active_archive}"
    page = int(page)
    ...
    texts = texts_of_reminders_with_numbers(lang=user.language, type=type_, reminders=reminders, page=page)
    reminder_text = "\n\n".join(texts[:5])
    len_of_reminders = length_of_reminders(len(reminders))
...
async def previous_page(callback: CallbackQuery):
    _, page, type_, active_archive = callback.data.split("_") # editpreviouspage__{page}_{type}_{active_archive}"
    page = int(page)

    user = await select_user(callback.from_user.id)
    reminders = await select_my_reminders(tg_id=callback.from_user.id, type = type_, active_archive=active_archive)

    texts = texts_of_reminders_with_numbers(lang=user.language, type=type_, reminders=reminders, page=(page-2))
...
async def edit_remind(callback : CallbackQuery):
    _, number, type_, active_archive, page = callback.data.split("_") #edit-remind_{i}__{type}_{active_archive}_{page}
    number, page = int(number), int(page)

    user = await select_user(callback.from_user.id)
    reminders = await select_my_reminders(tg_id=callback.from_user.id, type = type_, active_archive=active_archive)
    texts = text_of_reminders_number(lang=user.language, type=type_, reminders=reminders, number=number)
...
async def edit_remind(callback : CallbackQuery):
    _, number, page, type_, active_archive = callback.data.split("_") #deletereminder_{number}_{page}_{type}_{active_archive}'
    number, page = int(number), int(page)
...
async def confirm_delete(callback : CallbackQuery):
    _, number, page, type_, active_archive = callback.data.split("_") #confirmdelete_{number}_{page}_{type}_{active_archive}'
    number, page = int(number), int(page)
    ...
    texts = texts_of_reminders_with_numbers(lang=user.language, type=type_, reminders=reminders)
    reminder_text = "\n\n".join(texts[:5])
...
async def edit_remind(callback : CallbackQuery):
    ...
    _, number, page, type_, active_archive = callback.data.split("_") #deactivate_{number}_{page}_{type}_{active_archive}'
    number, page = int(number), int(page)
...
async def confirm_delete(callback : CallbackQuery):
    ...
    _, number, page, type_, active_archive = callback.data.split("_") #confirmdeact_{number}_{page}_{type}_{active_archive}'
    number, page = int(number), int(page)
...
async def edit_remind(callback : CallbackQuery):
    ...
    _, number, page, type_, active_archive = callback.data.split("_") #activate_{number}_{page}_{type}_{active_archive}'
    number, page = int(number), int(page)
...
def edit_texts_of_reminders(lang : str, type : str, reminders, number : int, desc=None, date=None, time=None, interval=None, start_time=None, end_time=None, yearly_date=None):
    ...
    if type == "onetime":
        ...
        description = reminder.description if desc is None else desc
        time_ = ", ".join(str(t.local_time) for t in reminder.times) if time is None else time
...
async def edit_remind(callback : CallbackQuery, state : FSMContext):
    _, number, page, type_, active_archive = callback.data.split("_") #editreminder_{number}_{page}_{type}_{active_archive}
    number, page = int(number), int(page)
    ...
    await state.update_data(my_first_mes_id = callback.message.message_id, my_data = callback.data)
...
async def edit_date(callback : CallbackQuery, state : FSMContext):
    ...
    _, __, ___, number, page, type_, active_archive = callback.data.split("_") #my_edit_name_{number}_{page}_{type}_{active_archive}
    number, page = int(number), int(page)

    cal = await make_calendar(user_data=user, change_date=True, 
    ...
    reminders = await select_my_reminders(tg_id=callback.from_user.id, type=type_, active_archive=active_archive)
    reminder_text = edit_texts_of_reminders(lang=user.language, type=type_, reminders=reminders,
                                            number=number, 
...
async def edit_info(callback : CallbackQuery, state : FSMContext):
    ...
    _, __, state_name, number, page, type_, active_archive = callback.data.split("_") #my_edit_name_{number}_{page}_{type}_{active_archive}
    number, page = int(number), int(page)

    state_data = await state.get_data()

    await state.update_data(my_first_mes_id = callback.message.message_id, my_data = callback.data, my_date_bool = True)
    ...
    await callback.message.edit_text(text=f'{headline}{ask_text}\n\n{reminder_text}', 
                                    parse_mode="Markdown",
...
async def update_my_reminder_field(lang, tg_id, state: FSMContext, message: Message, field: str = None, value: str = None):
    ...
    _, __, state_name, number, page, type_, active_archive = state_data["my_data"].split('_') #my_edit_name_{number}_{page}_{type}_{active_archive}
    number, page = int(number), int(page)

    reminders = await select_my_reminders(tg_id=tg_id, type=type_, active_archive=active_archive)

    reminder_text = edit_texts_of_reminders(lang=lang, type=type_, reminders=reminders,
                                            number=number, 
                                            desc = state_data.get("my_name"), 
                                            date = state_data.get("my_date"),
                                            yearly_date = state_data.get("my_yearly_date"),
...
async def update_my_reminder_field_callback(lang, tg_id, state: FSMContext, callback: CallbackQuery, field: str = None, value: str = None):
    ...
    await callback.message.edit_text(text=f'{headline}{lexicon_hdl[lang]["choose_edit"]}\n\n{reminder_text}', 
                                    reply_markup=await myrkb.edit_info_reminder(
                                            lang=lang,
                                            active_archive=active_archive,
                                            page=page,
...
async def edit_name(message: Message, state: FSMContext):
    ...
    reminders = await select_my_reminders(tg_id=message.from_user.id, type=type_, active_archive=active_archive)
    reminder_text = edit_texts_of_reminders(lang=user.language, type=type_, reminders=reminders,
                                            number=number, 
                                            desc = state_data.get("my_name"), 
                                            date = state_data.get("my_date"),
    ...
    await update_my_reminder_field(lang=user.language, tg_id=message.from_user.id, state=state, field="my_name", value=message.text, message=message)
...
async def edit_date(message : Message, state : FSMContext):
    ...
    await update_my_reminder_field(lang=user.language, tg_id=message.from_user.id, state=state, field="my_yearly_date", value=message.text, message=message)
...
async def edit_time(message : Message, state : FSMContext):
    ...
    await update_my_reminder_field(lang=user.language, tg_id=message.from_user.id, state=state, field="my_time", value=message.text, message=message)
...
async def edit_interval(message: Message, state: FSMContext):
    ...
    try:
        ...
        return
    await update_my_reminder_field(lang=user.language, tg_id=message.from_user.id, state=state, field="my_interval", value=message.text, message=message)
...
async def edit_start_time(message: Message, state: FSMContext):
    ...
    try:
        ...
        return
    await update_my_reminder_field(lang=user.language, tg_id=message.from_user.id, state=state, field="my_start_time", value=message.text, message=message)
...
async def edit_end_time(message : Message, state : FSMContext):
    ...
    try:
        ...
        return
    await update_my_reminder_field(lang=user.language, tg_id=message.from_user.id, state=state, field="my_end_time", value=message.text, message=message)
...
async def save_changes(callback : CallbackQuery, state : FSMContext):
    ...
    state_data = await state.get_data()
    _, __, ___, number, page, type_, active_archive = state_data["my_data"].split('_') # 'my_new_save_{number}_{page}_{type}_{active_archive}'
    number, page = int(number), int(page)
    ...
    for key, value in state_data.items(): 
                ...
                if value is not None:
                    match key:
                        case "my_name": update_fields["description"] = value
                        case "my_date": update_fields["date"] = value
                        case "my_time": 
                            if type_ == "onetime":
                                update_fields["remind_at"] = datetime.datetime.strptime(value, "%H:%M").time()
                            if type_ == "daily":
                                times = [datetime.time(hour = int(m), minute = int(k)) for m, k in [i.split(":") for i in value.split()]]
                                update_fields["times"] = times

                        case "my_interval": update_fields["interval_min"] = int(value)
                        case "my_starttime": update_fields["start_time"] = value
                        case "my_endtime": update_fields["end_time"] = value
                        case "my_yearly_date":
    ...
    match type_:
        case "onetime" :
            ...
            await update_onetime_reminder(reminder_id=reminder.id,
                                        **update_fields)

        case "daily" : 
            reminder = reminders[number]
            await update_daily_reminder(reminder_id=reminder.id,
                                        **update_fields)
            

        case "hourly" : 
            reminder = reminders[number]
            await update_hourly_reminder(reminder_id=reminder.id,
                                        **update_fields)
            
        case "yearly" : 
            reminder = reminders[number]
            await update_yearly_reminder(reminder_id=reminder.id,
                                        **update_fields)
